...
coefficientPressureSim = zeros(indexEndAirfoil - 1, 7);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for k = [3 5 9 19 29 49 99]
    
xSinkNew = zeros(1, k);
for l = [1 : length(xSinkNew)]
    xSink(l) = l / (k + 1);
end

yAirfoilNew = (t ./ 0.20) .* (0.2969 .* sqrt(xSinkNew) - 0.1260 .* xSinkNew - 0.3516 .* xSinkNew .^ 2 + 0.2843 .* xSinkNew .^ 3 - 0.1015 .* xSinkNew .^ 4);
    
M = zeros(k, k);
for j = [1 : k]
    for i = [1 : k]
        M(j, i) = atan2(yAirfoilNew(j), (xSinkNew(j) - xSinkNew(i)));
    end
end
...
vAirfoil = 0;
for i = [1 : k]
    uAirfoil = uAirfoil + sNew(i) .* (xyAirfoil(:, 1) - xSinkNew(i)) ./ ((xyAirfoil(:, 1)- xSinkNew(i)) .^ 2 + xyAirfoil(:, 2).^ 2);
    vAirfoil = vAirfoil + sNew(i) .* xyAirfoil(:, 2)./ ((xyAirfoil(:, 1) - xSinkNew(i)) .^ 2 + xyAirfoil(:, 2).^ 2);
end
